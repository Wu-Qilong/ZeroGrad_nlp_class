"""
solver.py — Text-Space Zero-Order Sudoku Solver 核心逻辑

算法流程（每次迭代）：
  Step 0  初始 prompt → LLM 生成第一份 solution_text
  Step 1  embed(current_text) → 语义向量 v ∈ R^d
  Step 2  采样单位球方向 u，构造 v± = v ± μu
  Step 3  project(v±) → LLM 根据扰动信号生成 text±
  Step 4  score(text±) → 解析并打分
  Step 5  ZO 梯度估计：grad_scalar = (f(x+μu) - f(x-μu)) / (2μ)
  Step 6  向量更新：v_new = v - lr * grad_scalar * u
  Step 7  project(v_new) → LLM 生成 new_text
  接受/拒绝：new_score < current_score → 接受；否则严格回滚
"""
...
Output ONLY the {size}x{size} grid. Each row on its own line, numbers 1–{size} separated by spaces."""
    ...
    def optimize(
        ...
        log(f"\n{'='*70}")
        log(f"Text-Space Zero-Order Optimization (ZeroGrad)")
        log(f"Board size: {size}x{size} | mu={mu} | lr={lr} | iters={n_iterations}")
        log(f"{'='*70}\n")

        # Step 0
        log("【Step 0】Generating initial solution via LLM prompt...")
        ...
        log(f"Initial score (lower=better): {current_score:.2f}")
        ...
        for iteration in range(n_iterations):
            ...
            log(f"  score(x+μu)={score_plus:.2f}  score(x-μu)={score_minus:.2f}")

            # Step 5: ZO 梯度估计
            grad_scalar = (score_plus - score_minus) / (2.0 * mu)

            # Step 6: v_new = v - lr * grad_scalar * u
            updated_vec = base_vec - lr * grad_scalar * u

            # Step 7: project(v_new) → new_text
            log(f"  grad_scalar={grad_scalar:.4f} → projecting updated vector → text_new ...")
            ...
            log(f"  new score={new_score:.2f}  (current={current_score:.2f}, best={best_score:.2f})")
            ...
            if new_score < current_score:
                ...
                log(f"  ✓ Accepted (improved by {improvement:.2f})")
                ...
                # 得分持平或变差 → 严格回滚
                log(f"  ✗ Rejected (score {new_score:.2f} ≥ current {current_score:.2f}), rolling back")
        ...
        log(f"\n{'='*70}")
        log(f"Optimization complete. Best score: {best_score:.2f}")
        log(f"{'='*70}")
    ...
    def solve(
        ...
        if is_valid_solution(best_board, size):
            print("✅ VALID SOLUTION!")
            print("\n【Completed Sudoku】")
            print_board(best_board)
            print_array(best_board)
            return best_board
        else:
            print("⚠️ PARTIAL SOLUTION (not fully valid)")
            print("\n【Current Board】")
            print_board(best_board)
            empty_count = sum(1 for row in best_board for c in row if c == 0)
            print(f"Empty cells remaining: {empty_count}/{size*size}")
            print_array(best_board)
            return None
